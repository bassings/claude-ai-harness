...
- **AC-QA-7:** A worst-case realistic review line (8 lenses in `lenses_run` and `verdicts`, `MAX_FINDINGS` findings, `ac_verdicts` filled to `MAX_AC_VERDICTS` with prefixed ids of a realistic multi-spec citation form) is written in full: `degraded` is absent, `findings` has `MAX_FINDINGS` entries and `ac_verdicts` has `MAX_AC_VERDICTS` entries in the stored line, both truncation counters are a real measured 0, and the remaining headroom is asserted as a number. This guards the collapse path in `ledger-append.mjs` (the byte-rescue ladder and the envelope-only degrade below it), which drops every finding, then every verdict, then the whole payload. Subsumes AC-OPS-6.

  **AMENDED AT FIX ROUND 4, on the round-3 review's H3, in lockstep with the guard rather than after it.** Two things in the original text were false, and the guard that answered it asserted neither of the properties it did state (it checked only that the record had not FULLY collapsed, while the line it produced held 0 of 15 findings and 189 of 200 verdicts with 32 bytes of headroom).
  - The fixture "prefixed ids of the widest accepted form" is not satisfiable and never was. Measured 2026-09-20 at fix round 4, varying only the prefix width: 30 characters writes whole with 24 bytes to spare, 31 sheds 2 findings, and 40 (the widest the pattern accepts at all) sheds all 15 findings and 7 verdicts. The criterion now names a realistic citation prefix instead, and the widest-accepted case is governed by the second guard below rather than pretended to fit.
  - The figures "15,600 bytes nulled and 12,200 bytes with ids retained" were taken before the `ac_verdicts` shrink loop and the tally field existed and do not reproduce. Re-measured at fix round 4 from a real written line: 8 lenses, 15 findings and 200 verdicts carrying a 25-character prefix serialise to **15,360 bytes against `MAX_LINE_BYTES` 16,384, so 1,024 bytes of headroom**, everything intact. The guard asserts a floor of 512 bytes, half the measured headroom, so a change that spends most of what is left fails with a number rather than silently shedding findings on the next busy round.

  The boundary itself is a second, separate criterion, because a byte budget has two facts and one assertion cannot carry both: past the budget (the 40-character prefix case) the record must shed in the documented order and COUNT what it shed -- `findings` first, then `ac_verdicts`, kept-plus-dropped accounting for every one, `lenses_run` and `verdicts` surviving, `degraded` never set. That is AC-DATA-13's ladder, asserted here as the behaviour at the edge.
