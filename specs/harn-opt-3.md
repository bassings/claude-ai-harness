...
- **Said of You runs production out of this repo under the OLD project name.**
  ...
  `name: birthday-memory-album`. Running containers are `birthday-memory-album`,
  `-auth`, `-db-1`, `-cloudflared-1`; only staging moved to
  `saidofyou-staging`. **`birthday-memory-album_pgdata` is the production
  database volume.** Never `docker compose up` against `docker-compose.yml`,
  and never "tidy" anything named `birthday-memory-album`. The
  `rollback-20260818-131211` tags on both services are the n-1 rollback path
