...
"does it exist"); it is non-empty with at least a markdown heading and a
section heading; and no file under `<repo>/.claude/` other than the report
itself was touched during the run (the optimiser is read-only with respect
to the repo it analyses -- see "Read-only in practice" below). The model's
free-text reply is still appended to the log as diagnostic context, but the
verdict never depends on it: a run that exits 0 and says nothing is a PASS
if the artefact is genuinely fresh, and a run that exits 0 and says the
right words is a FAIL if the artefact is stale, missing, empty, or the run
otherwise mutated the repo. Each repo gets one `RESULT PASS`/`RESULT FAIL`
...
**Verdict forgery**: every line this script itself writes -- the run
header, `exit=`, `SKIP`, `RESULT PASS`/`RESULT FAIL`, and the closing
`=== done ===` line -- carries a per-run nonce (`run=<hex>`), generated by
this script and never included in the prompt sent to `claude -p`. The
model's own stdout, captured into the same log file as diagnostic context,
can never carry that nonce, so a transcript that happens to print text
shaped like `RESULT PASS ...` cannot be mistaken for a real verdict by
anything that checks the nonce. The header line also carries
`version=$SCRIPT_VERSION`, bumped whenever this script's behaviour changes
materially, so the log shows which copy of the script actually produced a
given run -- the same drift class AC-OPS-4 already covers for `workflows/`,
...
governing budget is `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS`. The script now
sets it explicitly to 1,800,000ms (30 minutes -- real headroom over the two
measured 2026-08-16 runs, 515s and 311s) and aligns the outer `timeout` to
match (ceiling + 60s grace, `-k 60` to force a kill if claude ignores the
polite one). If a run hits the ceiling, the transcript contains
`CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` and the FAIL reason names it as the
cause rather than leaving an operator to infer it from a bare stale/missing
report.

**Permission constraints on the unattended job.** `claude -p` runs here
with `--disallowedTools` denying the destructive and outward-facing
operations this read-only job never needs: `Bash(rm:*)`, `Bash(sudo:*)`,
`Bash(git push:*)`, `Bash(git commit:*)`, `Bash(git reset:*)`,
`Bash(gh pr merge:*)`, `Bash(gh pr create:*)`, `Bash(gh issue create:*)`,
`Bash(gh release create:*)`, `Bash(gh workflow run:*)`, `Bash(curl:*)`,
`Bash(wget:*)`. `--permission-mode plan` was considered and rejected: plan
mode would prevent the optimiser writing the report this script's own
verdict depends on, silently trading a permission problem for a job that
runs and produces nothing. Verified empirically, end to end, against a
throwaway repo with both flags applied: a well-formed report was still
produced (exit 0), and nothing outside `.claude/optimise-cycle-report.md`
was created or modified.

**Read-only in practice, not just read-only in the skill's own contract.**
Measured 2026-08-16: the globally-installed conductor plan-guard Stop hook
touched `.claude/active-plan` in both delivery repos during the real run,
as a side effect of the background workflow the optimiser launches --
nothing this script does, but a mutation of the analysed repo all the
same. `claude -p` runs here with `--settings '{"disableAllHooks": true}'`,
which stops that hook firing for this one invocation (verified
empirically: `.claude/active-plan`'s mtime was unchanged after a real run
against a repo with the identical hook wired and an open-task plan
active). The verdict also asserts this as a postcondition independently of
whether the flag holds: any file under `<repo>/.claude/` other than the
report itself with an mtime at or after the run's own start time fails
that repo, naming the mutated file(s) in the reason.
